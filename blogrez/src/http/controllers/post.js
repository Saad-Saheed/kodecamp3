...
class PostController {
    index = asyncWrapper(async (req, res) => {
        let { tags, page, pageSize } = req.query;
        const { q } = req.query;
        ...
        const pagination = {
            ...
            previousPageUrl:
                page > 1
                    ? `${req.protocol}://${req.headers.host}/api/posts?page=${
                          page - 1
                      }`
                    : null,
            nextPageUrl:
    ...
    create = asyncWrapper(async (req, res) => {
        ...
        const validation = new Validator(newPost, rules);
    ...
    update = asyncWrapper(async (req, res) => {
        ...
        // validate request input
        const validation = new Validator(oldPost, rules);
